...
**Deniability over non-repudiation.** On the primary paths, messages are authenticated with the Double Ratchet's per-message key or HMAC-SHA256 (symmetric), not Ed25519 (asymmetric). Both peers can produce the same tag, so neither can mathematically prove the other authored a message. There is one exception to be aware of: when a message is delivered to a peer we are not directly connected to (multi-hop onion delivery, before a pairwise ratchet/HMAC session exists), it falls back to an Ed25519 signature, which IS attributable — so deniability holds for directly-established sessions but not for that multi-hop fallback path. Closing that gap (per-message forward secrecy and deniability on the fallback) is tracked as future work. Read receipts are an intentional Ed25519 exception because proving delivery is the point.
