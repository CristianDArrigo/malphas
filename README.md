...
**Deniability over non-repudiation.** Messages are authenticated with the Double Ratchet's per-message key or HMAC-SHA256 (symmetric), not Ed25519 (asymmetric). Both peers can produce the same tag, so neither can mathematically prove the other authored a message. This holds even when delivering to a peer we are not directly connected to: multi-hop delivery uses an **X3DH** session (seeded from the peer's signed prekey) that is forward-secret and deniable, seeding a Double Ratchet rather than signing cleartext. The only remaining non-forward-secret path is a legacy Ed25519 fallback used when we do not know the peer's signed prekey (e.g. a peer added via an old invite); import a current invite to avoid it. Read receipts are an intentional Ed25519 exception because proving delivery is the point.
